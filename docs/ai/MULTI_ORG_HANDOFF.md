...
| P4f (accounts and audit) | PR [#64](https://github.com/Dado211207/dvd-tivat-app/pull/64) **merged** into `main` as `c17a1723fb6740445f2f23d3913f0e42a5f66b7b` on 2026-09-25. Four migrations: `202609250033_audit_history.sql`; `202609250034_history_retention.sql`; `202609250035_current_row_identity.sql`; `202609250036_current_row_grants.sql`. 033–036 remain unapplied to production. |
| P4g (schema privilege hardening) | PR [#65](https://github.com/Dado211207/dvd-tivat-app/pull/65) — Stage A — **merged** into `main` as `35f85a7` on 2026-09-26. Migration `202609250037_service_role_trigger_grants.sql` revokes TRIGGER and REFERENCES from `service_role` on the thirteen P4f tables so the guards of 033–035 are the owner's alone to change. Green CI run `36175022662` on `75c4554`. Unapplied to production. |
| P5 (retire the DVD role mirror) | **Draft PR [#66](https://github.com/Dado211207/dvd-tivat-app/pull/66)** — Stage B — from `claude/dvd-tivat-app-dev-n8wctb-p5`. One migration `202609250038_retire_dvd_role_mirror.sql`; the branch now carries #65's merge (037) too. Independent review added three client-only fixes (flag-off membership-audit visibility; membership-derived role search; flag-off own-membership display on the Settings card). Not deployed. See the P5 section below. |
| P6–P8 | Not started. P6 (SZS interface) and P7 wait on Q1–Q8. |

**Production gate:** None of the organization rewrite migrations `202609240022` through `202609250038` has been applied to the hosted project (rechecked read-only on 2026-09-25 after #64 merged: 22 migrations, through `202609230021`; the hosted project is PostgreSQL 17.6), and the `send-web-push` function in these branches has not been deployed — the deployed worker is the one on `main`. A green local or PR test does not imply production deployment. Do not apply these migrations as a side effect of merging a PR. The plan requires separate approval per production phase. **Deploy order for P4e, when approved:** migration `202609250032` first, then the function — the new worker sweeps `push_delivery_queue()` and asks `push_delivery_verdict()`; without them its first read fails, it answers 503 and sends nothing. P4e changes one DVD behaviour on purpose (gate E5): an alert still queued when its call-out is closed or cancelled is no longer sent. Do not invent or delete member accounts or demo records.
...
## Follow-up `202609250037` (Stage A): only the owner changes the guards and constraints

Found in review of #64 at 036 - the "not decided" note below - and settled after #64 merged. Stage A, **merged** into `main` as `35f85a7` (#65). One migration `202609250037_service_role_trigger_grants.sql`; it does not change 033–036 and is **not deployed**.

**The gap it closes.** 033/034/036 took the write privileges off the seven audit, three history and three current-row tables, but the append-only, retention and identity rules of 033–035 are triggers owned by postgres, and the service role still held **TRIGGER** on all thirteen tables. `create or replace trigger` needs only the TRIGGER privilege, not ownership, so the service role could re-point a guard trigger at an existing permissive function it may execute and then make the very change the guard refuses. It also held **REFERENCES**, which is inert here (defining a foreign key also needs ownership of the altered table, and the service role owns none).

**What 037 does.** The service role loses TRIGGER and REFERENCES on all thirteen tables and keeps SELECT (and PG17's MAINTAIN). Only the owner can add, alter or drop a trigger or constraint on these tables. Evidence (`db-tests/service_role_trigger_grants.test.ts`, 7 tests): 3 of 7 fail without 037, 7/7 with it; five negative controls caught; suite 858/12; gate PASSED identical to the 036 run. Green CI run `36175022662` on `75c4554`.

## P5 (`202609250038`): retire the DVD role mirror; membership is the only authority

Stage B of the 2026-09-26 continuation, **draft PR [#66](https://github.com/Dado211207/dvd-tivat-app/pull/66)** from `claude/dvd-tivat-app-dev-n8wctb-p5`. One migration (`202609250038`), **not to be deployed yet**. After #65 merged, `main` was merged into this branch, so it now carries `037` as well (the harness runs `…036, 037, 038`). Independent review of #66 at `b9fa781` (pre-merge) then asked for two client-only fixes, added on top; the numbers below are the re-run on the merged + client-fix head — see the PR for the exact head SHA and CI run.

**What the mirror was.** Two copies of "who serves in DVD" were kept in step: `access_grants.role` and the DVD `organization_memberships` row. A trigger (`sync_dvd_membership_after_grant` → `sync_dvd_membership_from_grant()`) wrote the membership from the grant; the tail of `owner_set_organization_membership()` wrote the grant back; `owner_set_role()` set the grant and the trigger turned it into the membership. Authority already resolved through the membership (P3's `current_role_in`), and **no RLS policy reads `access_grants.role`**, so the grant's operational value was a second copy that could only agree or disagree.

**What 038 does.** Drops the trigger and its function; removes the DVD-to-grant mirror block from `owner_set_organization_membership()` (keeping the membership write and its audit tripwire); makes `owner_set_role()` baseline-only (PENDING/CITIZEN, refusing an operational role, tripwire kept). `access_grants` now carries only `active` and `OWNER` (+ the CITIZEN/PENDING baseline); a change to `access_grants.role` no longer moves anybody's operational access. Existing rows are left as they are — every operational account already has the membership the mirror wrote, so nobody loses access, and the residual grant value is inert (not rewritten).

**Client (Option B, owner-decided 2026-09-26; flag state unknown → correct under both).** DVD authority is read from `organization_memberships` unconditionally (`directory.ts`); the Accounts screen assigns DVD through `owner_set_organization_membership` (`setAccountRole`/`owner_set_role` no longer called from the client). SZS write controls stay gated by `VITE_MULTI_SERVICE_ADMIN_ENABLED` (P6). `NO_DVD_ROLE` renamed to `NO_SERVICE_ROLE` through the client and i18n. `docs/OWNER_BOOTSTRAP.md`: the ownership-transfer block demotes the outgoing owner to the CITIZEN baseline (the grant no longer carries operational authority; operational roles are memberships).

**Preserved (asserted in `db-tests/retire_role_mirror.test.ts`, 12 tests, failing-before/passing-after):** the owner keeps OWNER and full access with zero memberships and still needs a member record to act as a firefighter; suspending removes access in both services at once; existing DVD firefighters/commanders/admins keep their authority; a new account is a citizen the owner assigns through membership; a dual-service person has independent state per service. The four audit tripwires still fire.

**Fixture change, explained not disabled.** `db-tests/harness.ts`'s `grantRole` now writes the DVD membership itself (the trigger's old job), so every consumer that grants an operational role still establishes the same authority and audit history. The mirror-behaviour tests (`owner_service_membership`, `multi_service_accounts`, `recipient_organisation_scope`, `audit_triggers`, `access`, `bootstrap`, `restore_exact_function_text`) were updated to the new contract, not silenced.

**Evidence.** Failing-before 5/12 without 038, 12/12 with it. Three P5-specific negative controls each caught (DVD access loss, owner lockout, mirror left in place). Full DB suite **869 passed, 12 skipped** (862 P5 + 7 from 037's test); unit **678** (incl. the 4 new `AccountsView` tests); typecheck, lint, build, bundle check; `check:migrations` **39/39**; replay of 038 a no-op. **Production-copy gate PASSED** on the combined 037+038 tree, with the two intended P5 command changes classified as expected differences **E6** (`owner_set_role` refuses an operational role) and **E7** (`owner_set_organization_membership` no longer mirrors into the grant) — 0 divergences over 396 probes, **no new expected difference** from either 037 or the client fixes; the gate's 621-fact real-account read comparison is unchanged, which is itself the control that would catch an owner lockout or a DVD member losing access.

**Review follow-up (client, #66), three P5 regressions fixed on the branch — client-only, server audit policy and tripwire untouched.** (1) `loadOrganizationMembershipAudit()` was gated by `VITE_MULTI_SERVICE_ADMIN_ENABLED`, so with the flag off a DVD role change made on the Accounts screen — recorded since P5 in `organization_membership_audit`, not `role_audit` — was invisible in the owner's audit list; the read is now ungated (the server's `membership_audit_owner_read` policy already allowed the owner to see it), while SZS *assignment* stays flag-gated. (2) The Accounts search indexed `access_grants.role`, an inert legacy value after P5, so a stood-down firefighter still matched a FIREFIGHTER search; a new `roleSearchTerms()` (`directory.ts`) derives operational search terms from active memberships and keeps OWNER searchable. Failing-before/passing-after in `src/ui/views/AccountsView.test.tsx` (4 tests across flag-off and flag-on): 2 failed before the fix, 4 pass after. (3) The own-membership read for the signed-in person's Settings card (`loadOwnOrganizationMemberships` + `AccountAccessSetup`'s effect and state init) was also flag-gated, so with the flag off a DVD firefighter with an active membership was shown "no service" though the server gives them the FIREFIGHTER role. The read is now ungated for every signed-in account (`current_organization_memberships()` already scopes to `auth.uid()`); a failed read still shows the unavailable state, not "no service". Failing-before/passing-after in `src/ui/components/own-membership-display.test.tsx` (5 tests: DVD firefighter, dual-service, owner-with-none, read-failure, flag-on parity): 3 failed before the fix, 5 pass after.

**Replay note.** Because 038 rewrites `owner_set_role` and `owner_set_organization_membership` after restore file `202609230020`, re-applying 020 out of order reverts those two functions (it does not resurrect the dropped trigger/function). Sequential application keeps 038's versions; the restore-text test asserts both facts.
...
**P4f is done and merged; the P4 group is not complete.** #61, #62, #63 and #64 are merged into `main` (`c17a172`). Still open, in order of consequence: the cross-cutting existence question (whether another service's call-out id should be indistinguishable from an unknown one, plan P4 section); before `202609250034` is deployed, what happens to the published demo call-outs (the conflict with `docs/DEMO_DATA_INVENTORY.md` above - deleting them is now refused from 034, so it becomes an exceptional purge with its own approval). Settled: the three history tables are append-only, and a published call-out and its answer history stay (`202609250034`); citizen reports stay DVD-only as the abandoned feature, an explicit exception to criterion 5 (owner, 2026-09-25); an answer, the current journey step and the current availability can no longer be re-attributed, and a published call-out keeps every answer (`202609250035`); the service role reads those three tables and no longer writes them (`202609250036`). The TRIGGER/REFERENCES question is answered by `202609250037` (Stage A), **merged into `main` as `35f85a7` (#65)**: the service role loses TRIGGER and REFERENCES on all thirteen tables, so the guards of 033–035 become the owner's alone to change. P5 (retire the DVD role mirror, `202609250038`) is the open **draft #66** documented in the P5 section above; its branch has since taken `main` in by merge, so it now carries 037 as well. None of 033–038 is deployed.

**Who is watching:** the working session that opened #61–#64 was subscribed to all four, and to #65 (Stage A); each unsubscribed itself when its PR merged, #64 and #65 included. The only open draft now is P5 (#66); its session is subscribed to it. Whoever picks it up should re-subscribe or watch CI by hand.
...
2. P4f (migrations 033–036) is merged into `main` as `c17a172`, and **Stage A** (`202609250037`, service-role TRIGGER/REFERENCES) is merged as `35f85a7` (#65). **P5** (Stage B, `202609250038`, retire the DVD role mirror) is the open **draft #66**, its own migration and not deployed; its branch has taken `main` in by merge, so it carries 037 as well. Review it at its current head and require full CI on that exact SHA before merging. Production migration and push-worker deployment have separate gates. Each phase requires failing-before/passing-after database evidence, preserved DVD behavior (re-run `npm run gate:p4` against a fresh read-only capture) and CI success on the exact final SHA.
