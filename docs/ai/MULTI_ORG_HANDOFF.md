...
| P4f (accounts and audit) | **Draft PR [#64](https://github.com/Dado211207/dvd-tivat-app/pull/64)** from `claude/dvd-tivat-app-dev-n8wctb-p4f`, retargeted to `main` after #63 merged, which it has taken in by merge commit `61adab4` (a clean merge; no reviewed commit rewritten); not merged. Four migrations, each in its own commit: `202609250033_audit_history.sql` (the phase, `67d15b7`); `202609250034_history_retention.sql` (the owner's retention rule, `5743443`); `202609250035_current_row_identity.sql` (found in review of 034, `1011e32`); `202609250036_current_row_grants.sql` (found in review of 035, `a651bb5`). 034, 035 and 036 are not to be deployed yet. CI run `36163421686` (job `108165286695`) passed on `d366c9ff81dd79ea8a95f8ec373892af4ff834a5`, the first head carrying 036: lint, typecheck, unit 674/674, migration list 37/37, database 851 passed and 12 skipped (the hosted smoke test; CI holds no hosted credentials), build, bundle secret check and browser 334/334. Run `36158698603` passed on `4763175`, before 036. Later commits change documentation only; the PR records CI on its current head. |
