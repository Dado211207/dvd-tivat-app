...
| P5 (retire the DVD role mirror) | **Draft PR [#66](https://github.com/Dado211207/dvd-tivat-app/pull/66)** — Stage B — from `claude/dvd-tivat-app-dev-n8wctb-p5`. One migration `202609250038_retire_dvd_role_mirror.sql`; the branch now carries #65's merge (037) too. Not deployed. See the P5 section below. |
...
Stage B of the 2026-09-26 continuation, **draft PR [#66](https://github.com/Dado211207/dvd-tivat-app/pull/66)** from `claude/dvd-tivat-app-dev-n8wctb-p5`. One migration, **not to be deployed yet**. After #65 merged, this branch took `main` in by merge so it now carries 037 as well; the numbers below (862 db tests) are from before that merge, on 038 alone, and are re-run on the merged head.
...
**Evidence.** Failing-before 5/12 without 038, 12/12 with it. Three P5-specific negative controls each caught (DVD access loss, owner lockout, mirror left in place). Full suite **862 passed, 12 skipped**; unit 674; typecheck, lint, build, bundle check; `check:migrations` 38/38; replay of 038 a no-op. **Production-copy gate PASSED**, with the two intended P5 command changes classified as expected differences **E6** (`owner_set_role` refuses an operational role) and **E7** (`owner_set_organization_membership` no longer mirrors into the grant) — 0 divergences over 396 probes; the gate's 621-fact real-account read comparison is unchanged, which is itself the control that would catch an owner lockout or a DVD member losing access.
