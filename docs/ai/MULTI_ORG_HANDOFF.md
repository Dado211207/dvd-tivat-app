...
| P4f (accounts and audit) | PR [#64](https://github.com/Dado211207/dvd-tivat-app/pull/64) **merged** into `main` as `c17a1723fb6740445f2f23d3913f0e42a5f66b7b` on 2026-09-25 (merged by the owner). Four migrations, each in its own commit: `202609250033_audit_history.sql` (the phase, `67d15b7`); `202609250034_history_retention.sql` (the owner's retention rule, `5743443`); `202609250035_current_row_identity.sql` (found in review of 034, `1011e32`); `202609250036_current_row_grants.sql` (found in review of 035, `a651bb5`). Green CI run `36164801525` (job `108169830992`) on the merged head `5e90add`: lint, typecheck, unit 674/674, migration list 37/37, database 851 passed and 12 skipped, build, bundle secret check and browser 334/334. GitHub records no review object on #64; whether independent review happened elsewhere is the owner's to confirm. 033–036 remain unapplied to production. |
| P4g (schema privilege hardening, review of 036) | **Draft PR — Stage A** from `claude/dvd-tivat-app-dev-n8wctb-p4g`, based on `main` at `c17a172`. One migration `202609250037_service_role_trigger_grants.sql`: the service role kept TRIGGER and REFERENCES on the seven audit, three history and three current-row tables. `create or replace trigger` needs only TRIGGER (not ownership), so the service role could re-point an append-only/identity guard of 033–035 at a permissive function and defeat it; REFERENCES is inert (defining a foreign key also needs ownership of the altered table, and the service role owns none). Inspected first: nothing (app, `send-web-push`, the `pg_cron` worker running as `postgres`, Realtime via the WAL publication, the referential actions running as owner) needs either grant. 037 revokes both from the service role on all thirteen tables, keeps SELECT (and PG17's MAINTAIN), leaving the guards changeable by the owner alone. Not to be deployed yet. |
| P5–P8 | Not started. P5 (retire the DVD role mirror) is next; P6 and P7 wait on Q1–Q8. |

**Production gate:** None of the organization rewrite migrations `202609240022` through `202609250037` has been applied to the hosted project (rechecked read-only on 2026-09-25 after #64 merged: 22 migrations, through `202609230021`; the hosted project is PostgreSQL 17.6), and the `send-web-push` function in these branches has not been deployed — the deployed worker is the one on `main`. A green local or PR test does not imply production deployment. Do not apply these migrations as a side effect of merging a PR. The plan requires separate approval per production phase. **Deploy order for P4e, when approved:** migration `202609250032` first, then the function — the new worker sweeps `push_delivery_queue()` and asks `push_delivery_verdict()`; without them its first read fails, it answers 503 and sends nothing. P4e changes one DVD behaviour on purpose (gate E5): an alert still queued when its call-out is closed or cancelled is no longer sent. Do not invent or delete member accounts or demo records.
...
Found in review of #64 at 036 - the "not decided" note below - and settled after #64 merged. A **separate draft PR** (Stage A) from `claude/dvd-tivat-app-dev-n8wctb-p4g`, based on `main` at `c17a172`. One migration `202609250037_service_role_trigger_grants.sql`; it does not change 033–036 and is **not to be deployed yet**.

**The gap it closes.** 033/034/036 took the write privileges off the seven audit, three history and three current-row tables, but the append-only, retention and identity rules of 033–035 are triggers owned by postgres, and the service role still held **TRIGGER** on all thirteen tables. `create or replace trigger` needs only the TRIGGER privilege, not ownership, so the service role could re-point a guard trigger at an existing permissive function it may execute and then make the very change the guard refuses - the write revoke was only as firm as a trigger the service role could itself replace. It also held **REFERENCES**, which is inert here: defining a foreign key also needs ownership of the altered table, and the service role owns no relation, function, schema or type (verified read-only on production).

**Inspected before changing anything.** Nothing needs either grant: the application creates no triggers or constraints at run time and fires the existing ones through the security-definer commands regardless of privilege; `send-web-push` touches none of these tables; the `pg_cron` worker runs as `postgres` and only makes an HTTP call; Realtime reads the WAL through the `supabase_realtime` publication, not a service-role trigger (the only public-table triggers on production are the four the app defines, all owned by postgres); the referential actions into these tables run as the owner.

**What 037 does.** The service role loses TRIGGER and REFERENCES on all thirteen tables and keeps SELECT (and PostgreSQL 17's MAINTAIN, which cannot change a row or replace a trigger and is left as the platform default; it does not exist on PostgreSQL 16, where the suite and CI run, so it is not revoked). After it, only the owner can add, alter or drop a trigger or constraint on these tables - the guards of 033–035 become the owner's alone to change.

**Evidence** (`db-tests/service_role_trigger_grants.test.ts`, 7 tests). Failing-before on the 036 schema: the service role holds TRIGGER/REFERENCES on all thirteen and can re-point the append-only guard on `role_audit` and the identity guard on `intervention_responses` at a permissive function; 3 of 7 fail without 037, 7/7 with it. Passing-after proves every direct trigger/constraint change is `DENIED` while reads stay, the effective privileges are TRIGGER/REFERENCES false and SELECT true, `authenticated` keeps its read and `anon` never had one, the owner can still replace a guard, every command still runs for its real DVD and SZS callers with the guards firing, and 033–035's rules still answer. Five negative controls are each caught (keep TRIGGER on one table, keep REFERENCES on one, omit a table, over-revoke SELECT from the service role, collateral revoke from `authenticated`); a sixth - revoking from `postgres` - is inert, because an owner's privileges come from ownership, not the ACL, so `has_table_privilege` stays true and the owner can still create a trigger (which is why the owner check is behavioural). Full suite **858 passed, 12 skipped** (851 plus 7), no existing test changed; unit 674; typecheck, lint, build, bundle check clean; `check:migrations` 38/38; replay of 037 a no-op, identical to 036; production-copy gate **PASSED**, identical to the 036 run apart from nothing (037 changes no command).

**DVD behaviour.** None through the application. Only a direct trigger or constraint change by the service role is refused.

**At deployment,** check `has_table_privilege('service_role', <table>, 'TRIGGER')` and `'REFERENCES')` are both false for all thirteen tables and `'SELECT')` is still true. A privilege reaching the service role through another role would survive the revoke.
...
2. #64 (migrations 033–036) is merged into `main`. **Stage A** (`202609250037`, service-role TRIGGER/REFERENCES) is a draft PR on top of it, not deployed. **P5** (retire the DVD role mirror) is the next phase, its own migration in its own PR. Review each at its current head and require full CI on that exact SHA before merging. Production migration and push-worker deployment have separate gates. Each phase requires failing-before/passing-after database evidence, preserved DVD behavior (re-run `npm run gate:p4` against a fresh read-only capture) and CI success on the exact final SHA.
