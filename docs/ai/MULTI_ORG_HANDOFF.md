...
| P4d (attendance) | **Draft PR** from `claude/dvd-tivat-app-dev-n8wctb-p4d`, **stacked on #61** (base branch `claude/dvd-tivat-app-dev-n8wctb`); not merged. Migration `202609250031_attendance_service.sql`. CI does not run on a PR whose base is not `main`; local checks only until #61 merges and it is retargeted. |
