...
| P4g (schema privilege hardening) | **Draft PR [#65](https://github.com/Dado211207/dvd-tivat-app/pull/65)** — Stage A — from `claude/dvd-tivat-app-dev-n8wctb-p4g`, based on `main`. One migration `202609250037_service_role_trigger_grants.sql`: revokes TRIGGER and REFERENCES from `service_role` on the thirteen P4f tables so the guards of 033–035 are the owner's alone to change. Green CI run `36175022662` on `75c4554`. Not deployed. **Independent of P5 below**; whichever merges first, the other rebases `db-tests/harness.ts` (both append a migration line). |
| P5 (retire the DVD role mirror) | **Draft PR — Stage B** from `claude/dvd-tivat-app-dev-n8wctb-p5`, based on `main`. One migration `202609250038_retire_dvd_role_mirror.sql`. Not deployed. See the P5 section below. |
...
Stage B of the 2026-09-26 continuation, its own **draft PR** from `claude/dvd-tivat-app-dev-n8wctb-p5`, based on `main`. One migration, **not to be deployed yet**, independent of the P4g/037 draft (#65).
...
2. P4f (migrations 033–036) is **merged** into `main` as `c17a172`. Two independent draft PRs sit on top of it, each its own migration and neither deployed: **#65 (Stage A, `202609250037`)** revokes service-role TRIGGER/REFERENCES on the P4f tables; **P5 (Stage B, `202609250038`)** retires the DVD role mirror. Review each at its current head and require full CI on that exact SHA before merging; whichever merges first, rebase the other's one-line `db-tests/harness.ts` addition. Production migration and push-worker deployment have separate gates. Each phase requires failing-before/passing-after database evidence, preserved DVD behavior (re-run `npm run gate:p4` against a fresh read-only capture) and CI on the exact final SHA.
